...
def franquicia(franquicia): #'''Se ingresa la franquicia, retornando la cantidad de peliculas, ganancia total y promedio''' 
    ...
    return 'Franquicia:',franquicia, '- Cantidad:',cant, '- Ganancia_total:',total, '- Ganancia_promedio:',int(total/cant)
...
def productoras(productora): #'''Ingresas la productora, retornando la ganancia total y la cantidad de peliculas que produjeron'''
    ...
    return 'Productora:'+ productora,' - Ganancia_total: '+ str(total), '- Cantidad:' + str(cant)
...
def retorno(pelicula): #'''Ingresas la pelicula, retornando la inversion, la ganancia, el retorno y el año en el que se lanzo''' 
    ...
    return 'Pelicula:',pelicula, '- Inversion:',df['budget'][row], '- Ganacia:',df['revenue'][row],'- Retorno:',round(df['return'][row],2), '- Anio:',df['release_year'][row]
...
with gr.Blocks() as interface:
    with gr.Row():
        with gr.Column():
            gr.Label("Proyecto Individual",text_size=24, description="Made By: Gonzalo Schwerdt, Argentina")
            gr.Interface(fn=peliculas_dia, inputs="text", outputs="text", title="Peliculas Día", description="Ingresa un dia de la semana y devolverá todas las películas estrenadas dicho día. Ejemplo: 'viernes'")
            gr.Interface(fn=peliculas_mes, inputs="text", outputs="text",title="Peliculas Mes", description="Ingresa un mes del año y devolverá todas las películas estrenadas dicho año. Ejemplo: 'enero'")
            gr.Interface(fn=peliculas_pais, inputs="text", outputs="text",title="Peliculas País", description="Ingresa un pais y devolverá todas las películas estrenadas en dicho país. Ejemplo: 'US'")
            gr.Interface(fn=productoras, inputs="text", outputs="text",title="Productoras", description="Ingresa una productora y devolverá todas las películas estrenadas por la misma junto a la ganancia total. Ejemplo: 'Pixar Animation Studios'")
            gr.Interface(fn=retorno, inputs="text", outputs="text",title="Retorno", description="Ingresa el título de una película y devolverá su retorno junto a su inversion, ganancia, retorno y el año de la misma. Ejemplo: 'Toy Story'")
            gr.Interface(fn=franquicia, inputs="text", outputs="text",title="Franquicia", description="Ingresa una franquicia y devolverá la cantidad de películas que poseen, la ganancia total y la ganancia promedio por película. Ejemplo: 'Toy Story Collection'")
